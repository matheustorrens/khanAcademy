...
function setupMain() {
      ...
      if (responseObj?.data?.assessmentItem?.item?.itemData) {
        ...
        if (itemData.question.content[0] === itemData.question.content[0].toUpperCase()) {
          ...
          itemData.question.widgets = {
            "radio 1": { type: "radio", options: { choices: [{ content: "🍀", correct: true }] } }
          };
  ...
  (async () => {
    ...
    // Função unificada de clique por texto
    const clickByText = async (texts) => {
      for (const text of texts) {
        // Botões
        const btn = [...document.querySelectorAll('button')]
          .find(b => b.innerText.includes(text) || b.querySelector('span')?.innerText.includes(text));
        if (btn) { btn.click(); await delay(2000); }

        // Links
        const link = [...document.querySelectorAll('a')]
          .find(a => a.innerText.includes(text));
        if (link) { link.click(); await delay(2000); }
      }
    };
    
    while (window.khanwareDominates) {
      // Clicar nos selectors originais para resolver exercícios
      ...
      // Clicar automaticamente em "A seguir" e "Vamos lá" com delay maior
      await clickByText(["A seguir", "Vamos lá"]);

      // Delay rápido para exercício
      await delay(800);
    }
  })();
}
